...
const PurchaseOrderPage = () => {
  const [pos, setPurchaseOrders] = useState([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [resPurchaseOrders, resClients, resProjects] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        await axios.put(
          `http://127.0.0.1:8000/api/purchase-orders/${editId}`,
          payload,
          config,
        );
        Swal.fire({
        ...
        await axios.post("http://127.0.0.1:8000/api/purchase-orders", payload, config);
        Swal.fire({
          icon: "success",
          title: "Berhasil!",
          timer: 2000,
  ...
  const handleDelete = (id, code) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const token = localStorage.getItem("token");
          await axios.delete(`http://127.0.0.1:8000/api/purchase-orders/${id}`, {
    ...
    <div className="relative">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Kelola dan pantau semua pesanan beserta rincian itemnya.
          </p>
        </div>
        <button
        ...
        >
          <Plus size={20} /> Buat Purchase Order Baru
        </button>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-col md:flex-row gap-4 mb-6">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder="Cari klien atau nomor po..."
        ...
        <div className="w-full overflow-x-auto">
          <table className="w-full text-left table-auto min-w-max">
            <thead className="bg-gray-50 text-gray-500 text-xs uppercase font-bold border-b border-gray-200">
              <tr>
                <th className="py-4 px-6">ID Purchase Order</th>
                <th className="py-4 px-6">Klien</th>
                <th className="py-4 px-6">Jatuh Tempo</th>
                <th className="py-4 px-6 text-right">Grand Total Pesanan</th>
                  ...
                  <tr key={inv.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="py-4 px-6 text-center">
                      <div className="flex items-center justify-center gap-3">
                        <button
                          onClick={() => setSelectedPurchaseOrder(inv)}
                        className="text-blue-500 hover:text-blue-700 transition"
                        title="Lihat/Cetak Purchase Order"
                        >
                        <Eye size={18} />
                      </button>
                      <button
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl overflow-hidden my-auto max-h-[90vh] flex flex-col">
          <div className="bg-[#1A3263] px-6 py-4 flex justify-between items-center text-white shrink-0">
            <h3 className="font-bold text-lg">
              {isEditMode ? "Edit Purchase Order & Rincian" : "Buat Purchase Order Baru"}
            </h3>
            <button
                        ...
                        >
                          ...
                          <td className="p-2 text-center bg-yellow-50 border-l border-yellow-100">
                            <input
                              ...
                              title="Centang untuk memberi tanda di Purchase Order PDF"
                              disabled={isSpacing}
                            />
                          </td>
